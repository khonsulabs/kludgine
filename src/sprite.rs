...
impl Sprite {
    ...
    pub fn single_frame(atlas: &mut Atlas) -> Self {
        let size = atlas.size();
        let sprite_id = atlas.define_sprite(Rect::sized(0, 0, size.width, size.height));
    ...
    pub fn load_aseprite_json(raw_json: &str, atlas: &mut Atlas) -> KludgineResult<Self> {
        ...
        for (name, frame) in json["frames"].entries() {
            ...
            let sprite_id = atlas.define_sprite(frame);
...
impl SpriteFrameBuilder {
    pub fn new(sprite_id: AtlasSpriteId) -> Self {
        Self {
            sprite_id,
            tag: None,
            tag_frame: None,
